...
> 数据来源:run [33348304642](https://github.com/UsernameFull/transformers-ci/actions/runs/33348304642)(commit `3f040d7e80`,2026-08-31,含 torchcodec 源码构建 + npu_fixes 插件)。
...
| job | 收集 | 通过 | 失败 | 跳过 |
|---|---|---|---|---|
| common | 2102 | 1554 | 76 | 472 |
| trainer | 610 | 406 | 51 | 153 |
| models | 1394 | 801 | 48 | 545 |
| extra | 251 | 188 | 1 | 62 |
| **合计** | **4357** | **2949** | **176** | **1232** |

- 所有 `test_path` 内的测试文件 100% 被 pytest 收集(junit 中无缺失文件、无 collection error、无 deselected——`-k` 排除清单已于 2026-08-15 全部移除,策略为"全量放开,红即信号")。
- 三轮 run(33160513916 → 33238763483 → 33348304642)失败集合逐条稳定一致,修复可复现。
- `npu_fixes` 插件已消除 **DistNetworkError 16 个**(EADDRINUSE,xdist 双 worker 并发抢 29500)+ **deepspeed train_batch_size 4 个**(qwen3.5 复合模型嵌套 `AutoModel.from_config` 触发),models job 由 56 → 48。
- trainer 42 → 51:上游 8/28 后同步新增测试(APOLLO layerwise DDP 不支持、galore/FSDP 断言等),非插件回归,三轮逐条一致。
...
| 家族 | 数量 | 归因 | 解除条件 |
| --- | --- | --- | --- |
| **ACL-500001** `AclSetCompileopt ... internal ACL of the system is incorrect` | ~47(trainer 30 / common 12 / models 5) | **宿主机驱动/CANN 异常**,非代码问题。证据:①失败点均为各进程第一个 NPU 算子调用(adam 优化器、`tensor.to("npu")`、普通比较运算等互不相关的操作);②双卡同时中招;③独立 run 失败集合逐条一致(确定性复现);④8月24日同环境曾全绿 | 上机排查:`npu-smi info` → `fuser -v /dev/davinci*` 清理残留进程 → 设备复位/重启宿主机;核对宿主 driver/firmware 与容器 CANN 9.0.0 版本配对 |
...
| deepspeed resize embeddings IndexError(size 0) | 12 | meta-device 下 padding_idx 归零逻辑踩空;MASTER_PORT 修复后测试跑到更深处,IndexError 成为最终暴露的上游缺陷 | 等上游 |
| APOLLO layerwise DDP 不支持等上游新增断言 | ~9 | 上游 8/28 后新增测试(APOLLO_ADAMW_LAYERWISE DDP、galore layerwise、FSDP non-rank0 等)在 NPU 上的固有缺陷 | 等上游 |
| FSDP/TP spawn 失败 | ~11 | 多进程分布式测试在多卡/容器环境下的偶发失败 | 观察 |
...
| flaky / 上游抖动 | 若干 | 如 paligemma 可视化测试偶发下载卡死 2h+、`test_length_warning_assisted_generation` 隔日翻转 | 观察;必要时加 pytest-timeout |
...
| NPU 修复插件 | workflow `Write NPU test fixes plugin` 步骤 | 内联生成 `npu_fixes.py`:①按 xdist worker 分配唯一 `MASTER_PORT`(29500+idx×1000)并设 `MASTER_ADDR/RANK/WORLD_SIZE/LOCAL_RANK`,让 deepspeed 走 env:// 路径(否则 mpi_discovery 硬编码 29500),消除 EADDRINUSE;②monkeypatch `HfDeepSpeedConfig.__init__` 注入缺失的 `train_micro_batch_size_per_gpu=1`,修复 qwen3.5 复合模型嵌套 `AutoModel.from_config` 的 zero3 config 断言 |
